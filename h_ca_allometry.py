# gives height to area for all trees
# to get only for trees in competition uncomment end
import pandas as pd
import numpy as np
import torch
import cv2
...
fname = join(train_dir, "SJER-train.csv")
print("\n\n\n")
print("Reading csv...")
...
# uncomment to get only for trees in competition
#indices_to_keep = []
...
comp_df = df
print("Saving csv...")
bname = basename(fname).split("-")[0].lower()
comp_df.to_csv("area_ht_" + bname + ".csv", header=True, index=False)
