...
np.random.seed(42)
n_classes = 1
rules = [FOL_competition(device, 1, None, None), ] #[FOL_bbox_2big(device, 1, None, None), ]   #[FOL_green(device, 2, None, None), ]
rule_lambdas = [1]
pi_params = [0.95, 0.4]  #0.9, 0
batch_size = 1
C = 1
...
#######################################################################################################################################################################################################
# log data to locally
log_fname = "df_reg_log_low_var.csv".format(args.site)
