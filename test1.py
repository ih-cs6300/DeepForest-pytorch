...
#######################################################################################################################################################################################################
# log data to locally
log_fname = "df_niwo_junk_log.csv"
if not (os.path.isfile(log_fname)):
   f = open(log_fname, "w")
   f.write("train,test,epochs,seed,bbox_prec,bbox_rec,class_prec,class_rec\n")
   f.close()

f = open(log_fname, "a")
f.write("{},{},{},{},{},{},{}\n".format(train_csv, test_csv, m.config["train"]['epochs'], args.seed, results['box_precision'], results['box_recall'], results['class_recall']['precision'].item(), 
          results['class_recall']['recall'].item()))
f.close()
######################################################################################################################################################################################################
