...
# for fixed pi_f
#csv_wr_obj = mlg.Writer(pars.args.log, ["exp_id", "site", "seed", "pi_f", "rule", "epochs", "chm", "train", "test", "bbox_prec", "bbox_rec", "class_prec", "class_rec"])
#csv_wr_obj.write_data([comet.experiment.id, pars.args.site, str(pars.args.seed), str(pars.args.pi_f), "R2", str(pars.args.epochs), pars.args.chm, train_csv, test_csv, results['box_precision'], results['box_recall'], results['class_recall']['precision'].item(), results['class_recall']['recall'].item()])
